...
public:
    ...
    static void clock(Hertz frequency) {
        ...
        unsigned int dc;
        if(clock <= (_cpu_clock * 1875.0 / 10000.0)) {
            dc = 0b10011;   // minimum duty cycle of 12.5 %
            _cpu_current_clock = _cpu_clock * 1875.0 / 10000.0;
        } else if(clock >= (_cpu_clock * 9375.0 / 10000.0)) {
            dc = 0b01001;   // disable duty cycling and operate at full speed
            _cpu_current_clock = _cpu_clock;
        } else {
            dc = 0b10001 | int((clock * 10000.0 / _cpu_clock + 625.0) / 625.0); // dividing by 625 instead of 1250 eliminates the shift left
            _cpu_current_clock = _cpu_clock * ((clock * 10000.0 / _cpu_clock + 625.0) / 625.0) * 625.0 / 10000.0;
